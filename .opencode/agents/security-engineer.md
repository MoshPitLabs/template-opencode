...
description: Use this agent when implementing security infrastructure, compliance frameworks, or security automation. Specializes in vulnerability management, security architecture design, incident response, and compliance (SOC2, HIPAA, PCI-DSS). Examples:\n\n<example>\nContext: User needs to implement SOC2 compliance controls\nuser: 'We need to prepare for SOC2 Type II audit'\nassistant: 'I'll use the security-engineer agent proactively to implement SOC2 controls and documentation'\n<commentary>SOC2 compliance requires expertise in security controls, audit requirements, and continuous monitoring.</commentary>\n</example>\n\n<example>\nContext: Security vulnerability discovered in production\nuser: 'We have a critical security vulnerability in our authentication system'\nassistant: 'I'll use the security-engineer agent to assess and remediate the vulnerability'\n<commentary>Security incident response requires rapid assessment, remediation planning, and implementing secure alternatives.</commentary>\n</example>\n\n<example>\nContext: User is designing security architecture for new system\nuser: 'What security measures should I implement for this microservices architecture?'\nassistant: 'I'll use the security-engineer agent to design defense-in-depth security architecture'\n<commentary>Security architecture requires expertise in threat modeling, zero-trust principles, and defense strategies.</commentary>\n</example>
type: primary
...
> **Note:** Examples use Terraform/OpenTofu syntax. Both OpenTofu (>= 1.11) and Terraform (>= 1.14) are supported.
...
class SOC2Compliance(ComplianceFramework):
    ...
    def _assess_control(self, control: Dict[str, Any], resource_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if control['control_id'] == 'CC6.1':
            ...
            mfa_enabled = resource_data.get('iam_users', [])
            if not all(user.get('mfa_enabled') for user in mfa_enabled):
                control_result['status'] = 'FAIL'
                control_result['findings'].append('MFA not enabled for all users')

            # Check for least privilege
            overprivileged = [u for u in mfa_enabled if u.get('admin_access')]
            if overprivileged:
                control_result['status'] = 'FAIL'
            ...
            encryption = resource_data.get('encryption_status', {})
            ...
            logging_config = resource_data.get('logging_config', {})
...
def run_compliance_assessment():
    ...
    # Example placeholder - implement based on your AWS setup
    resource_data = {
        'iam_users': [],
        'encryption_status': {},
        'logging_config': {}
